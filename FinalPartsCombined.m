function FinalPartsCombined(patientHeight, hipShaft, dorsiCablePosition, plantarFlexionPosition, plantarTorsionSpring, dorsiTorsionSpring)
...
%% HipPadding.txt Variables 
    ...
    vHipPadding = lengthBackBar*(paddingOuterDiameter-paddingInnerDiameter)*hipPaddingThickness + ...
...
%% Hip2DOFJointBearing.txt Variables 
    ...
    vHip2DOFJointBearing = pi*((((0.0269875/1.78)*patientHeight)^2)-(((0.0225925/1.78)*patientHeight)^2))*((0.008573/1.78)*patientHeight) + ...
        pi*(((0.0186835/1.78)*patientHeight)^2)-(((0.0147635/1.78)*patientHeight)^2)*(((0.008573/1.78)*patientHeight)) + ...
        (4/3)*pi*(((0.00288057/1.78)*patientHeight)^3)*12;
...
%% Hip2DOFJointShaft.txt Variables 
    ...
    vHip2DOFJointShaft = pi*((shaftDiameter1/2)^2)*shaftLength + ...
...
%% Hip2DOFJointOutputShaft.txt Variables 
    ...
    vHip2DOFJointOutputShaft = cutWidth*(connectorLength+taperLength+2*radiusHipCylinder)*shaftDiameter1 + pi*radiusHipCylinder^2*shaftDiameter1 + ...
        2*pi*((pinDiameter/2)^2)*pinLength - ...
        connectorLength*((0.00694951/1.78)*patientHeight)*shaftDiameter1 + ...
...
%% Hip2DOFJointCasing.txt Variables 
    ...
    vHip2DOFJointCasing = pi*(casingOuterDiameter/2)^2*casingExtrudeLength - ...
...
%% MedialDiskBallBearing.txt Variables 
    ...
    vMedialDiscBallBearing = pi*(((0.0269875/1.78)*patientHeight)^2 - ((0.01918775/1.78)*patientHeight)^2)*(0.01/1.78)*patientHeight + ...
        pi*(((0.01527975/1.78)*patientHeight)^2 - ((0.007956/1.78)*patientHeight)^2)*(0.01/1.78)*patientHeight + ...
        12*(4/3)*pi*((0.00576114/1.78)*patientHeight)^3;
...
%% HipMedialDisk.txt Variables 
    ...
    vHipMedialDisc =  pi*(((0.035512/1.78)*patientHeight)^2 - ((0.018512/1.78)*patientHeight)^2)*medialDiskWidth + ...
...
%% Paramaterized Pulley 
    ...
    vParametrizedPulley = 2*pi*((0.015875/1.78)*patientHeight)^2*((0.001924/1.78)*patientHeight) + ...
...
% dorsiflexion torsional cam dimensions  
    ...
    vDorsiCamDimensions = 2*pi*(OuterDorsiCamRadius^2)*DorsiCamCutDepth + ...
        pi*(DorsiCamRadius^2)*(DorsiCamWidth - (2*DorsiCamCutDepth)) - ...
        pi*InnerDorsiCamDiameter^2*(DorsiCamWidth + (2*DorsiCamCutDepth)) - ...
        pi*((OuterCutDiameter^2)-(InnerCutDiameter^2))*(DorsiCamWidth + (2*DorsiCamCutDepth)) + ...
        (DorsiCamWidth + (2*DorsiCamCutDepth))*DorsiKeyWidth*(DorsiArm-(2*OuterDorsiCamRadius)) - ...
        pi*DorsiKeyWidth^2*DorsiCamCutDepth;
    ...
    vDorsiCamShaft = pi*(DorsiCamShaftDp1/2)^2*DorsiCamZ2 + ...
...
% 4-bar bolt dimensions
    ...
    vPlantarCamBearing = pi*(((0.009777/1.78)*patientHeight)^2-((0.00711475/1.78)*patientHeight)^2)*((0.004983/1.78)*patientHeight) + ...;
    ...
    vDorsiCamBearing = pi*(((0.00985/1.78)*patientHeight)^2-((0.00718906/1.78)*patientHeight)^2)*((0.004983/1.78)*patientHeight) + ...;
    ...
    vCamRetainingRing = 2*(pi*(CamretainingRingOuterDiameter^2-CamretainingRingInnerDiameter^2)*CamretainingRingThickness + ...
    ...
    vDorsiRetainingRing1 = 2*(pi*(DorsiretainingRingOuterDiameter1^2-DorsiretainingRingInnerDiameter1^2)*CamretainingRingThickness + ...
    ...
    vDorsiRetainingRing2 = 2*(pi*(DorsiretainingRingOuterDiameter2^2-DorsiretainingRingInnerDiameter2^2)*CamretainingRingThickness + ...
        2*DorsiretainingRingHolediameter*((0.0026096/1.78)*patientHeight)*DorsiretainingRingThickness - ...
        2*(DorsiCircleOffset^2)*DorsiretainingRingThickness);
    ...
    vPlantarCam = 2*(pi*(((0.02292/1.78)*patientHeight)^2-((0.01429556/1.78)*patientHeight)^2)*(0.001/1.78)*patientHeight) + ...
    ...
    vPlantarCamShaft = pi*(PlantarCamDp1/2)^2*PlantarCamZ1_Z2 + ...
    ...
    vBotBar = TbotBar*(HbotBarKnee+HbotBarAngle+HbotBarCOM)*WbotBar + ...
    ...
    v4BarBolts = pi*(boltDiameter/2)^2*bolt1Length + ...
    ...
    vTopBar = TtopBar*(HtopBarHip+HtopBarAngle+HtopBarKnee)*WtopBar + ...
    ...
    vCalfCase = (pi/2)*(calfOuterCaseDiameter^2-calfInnerCaseDiameter^2)*calfCaseHeight + ...
        QuartercalfCaseHeight*(2*distToCutX)*(0.02110317/1.78)*patientHeight + ...
        QuartercalfCaseHeight^2*((0.00805474/1.78)*patientHeight) - ...
        (1/2)*calfInnerCaseDiameter*HalfcalfCaseHeight*(calfOuterCaseDiameter/2);
    ...
    vCalfPadding = (pi/2)*((calfOuterPaddingDiameter/2)^2-(calfInnerPaddingDiameter/2))^2*calfPaddingHeight - ...
    ...
    vThighCase = (pi/2)*((thighOuterCaseDiameter/2)^2-(thighInnerCaseDiameter/2)^2)*thighCaseHeight + ...
        QuarterthighCaseHeight^2*((0.0313673/1.78)*patientHeight) - ...
        (1/2)*thighInnerCaseDiameter*((0.06/1.78)*patientHeight)*(thighOuterCaseDiameter/2);
    ...
    vThighPadding = (pi/2)*((thighOuterPaddingDiameter/2)^2-(thighInnerPaddingDiameter/2^2))*thighPaddingHeight - ...
        (1/2)*thighInnerPaddingDiameter*((0.06/1.78)*patientHeight)*(thighOuterPaddingDiameter/2);     
